...
- Mantle hotspots (formation of volanic island belts)
...
Currently, (I think) `tectonic-sim` does a decent job with forming believeably realistic looking plate geography over time. 

A couple of highlights:
- Continental-continental crust collisions properly dampen momentum for involved plates, which also result in mass transfer, crust thickening, and sometimes plate fusion (e.g. Indian and Eurasian plates -> Tibetan Plateau)
- Different plate collisions properly handle subduction patterns (even though we dont have a density/buoyancy model). Denser oceanic crusts subduct under less dense continental crusts, while at oceanic-oceanic boundaries, crust age often determines its density and subduction
- The world generates new young crust mass at diverging plate boundaries, even at continental rifts (e.g. East African Rift)
...
For this module, the initial conditions of the simulation is often what dominates the final look of the plates, since the simulation environment spans a limited time window. 

To start, I am using a fairly naive Voronoi tessellation method to partition the torus world into distinct plates, basically assigning each cell to the nearest plate origin point using their torus-aware distances. 

However, a plain nearest-seed partition produces obviously artificial boundaries: straight edges and tidy convex polygons. So we can distort the distances with a domain-warp and use weighted-Voronoi to make the plate boundaries more irregular and natural, as well as variation in plate sizes. 

![Voronoi partitioned plates (not on torus)](./voronoi.png){h=400}
...
- Rifting regions will often create dozens of independent microplates. This is okay for the most part, but dont fuse often enough with neighboring plates. 
- The noise function used in perturbing the initial topography of continental plates are almost too noisy, often creating excess amounts of basins and inland seas, fragmenting large pieces of continental mass. 
...
There are a number of interesting phenomenon that could be integrated into the sim for more accurate and realistic output results: 
...
- Based on how limiting the current pseudo-physics interpretation of tectonics are for future work, we could make a case for a full 3D physics simulation.
...
- No straight or near-straight edges in coastlines or elevation contours longer than some threshold. This one would catch most of the engine-bug artifacts I've run into directly.
